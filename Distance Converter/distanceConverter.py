...
class MetresToFeet(ttk.Frame):
    def __init__(self,container,controller):
        ...
        metres_input = ttk.Entry(self,textvariable=self.metres_value,width=10,)
        metres_input.grid(row=1,column=2,sticky='EW')
        metres_input.focus()

        feet_label = ttk.Label(self,text="Feet: ")
        feet_label.grid(row=2,column=1,sticky='W',ipadx=5)
        feet_display = ttk.Label(self,textvariable=self.feet_value)
        feet_display.grid(row=2,column=2,sticky='EW')

        calculate_button = ttk.Button(self,text="Calculate", command=self.calculate)
        calculate_button.grid(row=3,column=1,columnspan=2,sticky='EW')
